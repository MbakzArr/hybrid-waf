...
\TODO{This section still needs the full per-category attack breakdown
the moderator asked for (how much of the attack traffic is SQLi versus
XSS versus path traversal versus other, across the merged dataset). That
requires a script run against the raw CSIC 2010 and HTTPParams files
that has not been done yet. What follows is the composition already
confirmed by build\_dataset.py; the category-level breakdown is the
open item.}
