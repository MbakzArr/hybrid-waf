...
\definecolor{ujnavy}{RGB}{0,42,82}
\definecolor{ujblue}{RGB}{61,139,205}
\usepackage{booktabs}
\usepackage{amsmath}
\usepackage{xcolor}
...
rule-only and an ML-only configuration, with ModSecurity and OWASP
Core Rule Set v4 stood up as an external baseline on the same test set.
...
and 0.568 for ModSecurity with CRS v4, with a false positive rate of
0.6\% against zero for the rule engine alone and 4.5\% for CRS,
meaning the small false positive cost of adding the classifier
bought a large recall improvement, and the hybrid configuration
outperformed a fully deployed industrial rule set on both metrics
simultaneously. The classifier caught obfuscated
...
\begin{tikzpicture}[
  ...
  box/.style={draw=ujnavy, fill=ujnavy, text=white, rounded corners,
              minimum width=2.4cm, minimum height=1.05cm,
              align=center, font=\small\bfseries, line width=0.4pt},
  detbox/.style={draw=ujblue, fill=ujblue, text=white, rounded corners,
              minimum width=2.4cm, minimum height=1.05cm,
              align=center, font=\small\bfseries, line width=0.4pt},
  arr/.style={-{Latex[length=2.2mm]}, thick, draw=ujnavy}
]
\node[box] (m1) {M1\\TrafficReader};
\node[box, right=of m1] (m2) {M2\\FeatureExtractor};
\node[detbox, above right=8mm and 14mm of m2] (m3) {M3\\RuleEngine};
\node[detbox, below right=8mm and 14mm of m2] (m4) {M4\\MLClassifier};
\node[box, right=26mm of m2] (m5) {M5\\DecisionEngine};
\node[box, right=of m5] (m6) {M6\\Logger};
...
\node[fit=(m3)(m4)(m5), inner sep=7mm, draw=ujnavy, dashed,
      rounded corners, line width=0.5pt] (modebox) {};
\end{scope}
\node[above=1.5mm of modebox, font=\scriptsize\itshape, text=ujnavy]
     {selectable detection paths, hybrid mode consults both};
\end{tikzpicture}%
}
\caption{Six-module pipeline. \texttt{rule\_only} consults M3 alone,
\texttt{ml\_only} consults M4 alone, \texttt{hybrid} consults both and
resolves through M5.}
\label{fig:pipeline}
\end{figure}

Figure~\ref{fig:pipeline} shows the pipeline. M2 branches into M3 and
M4, which run independently of each other and are the two paths an
operating mode can selectively skip, before both results converge on
M5 for the decision, and M6 records the outcome.
...
A third dataset, WADE-2022, referenced in earlier project planning as
a source of modern attack traffic, could not be integrated. No public
download location or verifiable academic source for a dataset under
that name could be located, and it was therefore excluded from this
evaluation, which relies on CSIC 2010 and HTTPParams only.
...
correct. ModSecurity 2.9.7 with OWASP CRS v4.10.0 was stood up as an
external baseline on the project VM (Apache 2.4.58, default paranoia
level 1, default anomaly score threshold of 5) and evaluated against
the identical 25{,}627-request test set, each request sent as a
URL-encoded query parameter value over HTTP, with a 403 response
counted as a block.
...
ModSecurity + CRS v4    & 0.799 & 0.440 & 0.568 & 0.045 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
  ...
  symbolic x coords={Rule-only, ModSec+CRSv4, Custom LR, Random Forest, Hybrid},
  xtick=data,
...
\addplot[fill=ujnavy, draw=ujnavy] coordinates {
  (Rule-only,1.000) (ModSec+CRSv4,0.799) (Custom LR,0.651)
  (Random Forest,0.980) (Hybrid,0.980)};
\addplot[fill=ujblue, draw=ujblue] coordinates {
  (Rule-only,0.289) (ModSec+CRSv4,0.440) (Custom LR,0.423)
  (Random Forest,0.713) (Hybrid,0.714)};
\addplot[fill=gray!55, draw=gray!70] coordinates {
  (Rule-only,0.449) (ModSec+CRSv4,0.568) (Custom LR,0.512)
  (Random Forest,0.825) (Hybrid,0.826)};
\legend{Precision, Recall, F1}
\end{axis}
\end{tikzpicture}
\caption{Detection performance across all five configurations on the
25{,}627-request test set. The rule engine alone reaches perfect
precision but the lowest recall of any configuration. The hybrid
configuration achieves the highest F1-score, above both the ModSecurity
with CRS v4 external baseline and either detection path alone.}
\label{fig:results}
\end{figure}
...
The comparison against ModSecurity with CRS v4 is a notable result in
its own right. The hybrid configuration reaches an F1-score of 0.826
against CRS's 0.568 on the same test set, and does so with a lower
false positive rate, 0.6\% against CRS's 4.5\%. This is not a comparison
against a weak baseline: CRS is the reference open-source rule set the
wider field measures against, and its recall here, 0.440, sits below
both machine-learning configurations in this table, meaning it misses
more attacks in this test set than either the custom logistic
regression or the Random Forest classifier does on its own. The
elevated CRS false positive rate is also consistent with
\citet{reyesnarvaez2025}, who report exactly this trade-off at CRS's
higher paranoia levels, false positives rise as coverage widens. Taken
together, this suggests the gap this project targets, a narrow, fast
rule engine paired with a lightweight classifier, is a real gap even
against a fully deployed industrial rule set, not only against the
36-rule engine built for this project.
...
rules, at a false positive cost of 0.6\% on benign traffic. ModSecurity
with CRS v4 lets 56.0\% of attacks through unblocked on this same test
set, worse than either machine-learning configuration and better only
than this project's own rule-only mode, reinforcing that the hybrid
result is not an artefact of comparing against an unusually weak rule
engine.
...
0.826 against 0.449 for the rule engine alone and 0.568 for
ModSecurity with OWASP CRS v4 stood up as an external baseline, at a
false positive rate of 0.6\% against 4.5\% for CRS, and the classifier
caught obfuscated attacks the rule engine missed on 12.61\% of test
requests, this project's core Case B contribution. Reframed as attacks
reaching the backend unblocked, the hybrid configuration reduced that
figure from 71.1\% under rule-only protection to 28.6\%, compared with
56.0\% for ModSecurity with CRS v4. This is a direct, positive answer
to this paper's research question: a lightweight classifier added to a
rule-based WAF measurably improves detection without discarding the
rule engine's speed and transparency, and does so to a degree that
outperforms a fully deployed industrial rule set on this test set,
though the choice of how to
...
than implied otherwise. The WADE-2022 dataset referenced in earlier
planning could not be located and was not used. The decoding pass and
continual learning capabilities described in this project's design
documents have been prototyped and measured separately but not yet
integrated into the repository evaluated here. The observed
false-positive behaviour on legitimate SQL-like text
(Section~\ref{sec:prelim}) is reported as a structural limitation of
the current feature set rather than something this alpha version
resolves.

Toward the Beta version, the immediate priority is integrating the
decoding pass, which raised Random Forest F1 from 0.814 to 0.833 in
isolated testing, since it directly addresses the obfuscation case
this project is built around. Locating a genuine third dataset with
modern encoded and obfuscated attack traffic, to replace the WADE-2022
reference that could not be substantiated, is a secondary priority.
